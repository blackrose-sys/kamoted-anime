...
export function ResetPassword() {
  ...
  useEffect(() => {
    // Supabase JS client automatically detects the #access_token in the URL hash
    // and fires PASSWORD_RECOVERY. We simply listen for it.
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'PASSWORD_RECOVERY' && session) {
        setReady(true);
        setError('');
      } else if (event === 'SIGNED_IN' && session) {
        // Also handle if it fires as SIGNED_IN instead of PASSWORD_RECOVERY
        setReady(true);
        setError('');
      }
    });

    // Fallback: if Supabase already processed the hash before this mounted
  ...
  // Loading state while waiting for Supabase to process the token
  if (!ready && !error) {
    return (
